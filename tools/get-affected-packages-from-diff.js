...
function CiSkipBasedOnDiffDisabled(filePaths) {
  ...
  if (!checkIfNoDiff(filePaths)) {
    ciSkipBasedOnDiffDisabled = false;
...
// this field should be set as true to get the list of affected packages
// setting this false returns an empty array in case we want to disable the
// package testing/building with just a job level env variable
const CI_SKIP_BASED_ON_DIFF_DISABLED =
  process.env.CI_SKIP_BASED_ON_DIFF_DISABLED;

let getAffectedPackagesFromDiff = new Map();

if (CI_SKIP_BASED_ON_DIFF_DISABLED == "false") {
  getAffectedPackagesFromDiff.set("ciSkipBasedOnDiffDisabled", false);
  ...
  if (RUN_CI_FOR_AFFECTED_PACKAGES_FROM_DIFF_ENABLED == "true") {
    ...
    getAffectedPackagesFromDiff.set(
      "ciSkipBasedOnDiffDisabled",
      CiSkipBasedOnDiffDisabled(gitDiffFilePaths),
    );
    ...
    getAffectedPackagesFromDiff.set("ciSkipBasedOnDiffDisabled", true);
    getAffectedPackagesFromDiff.set(
      "affectedPluginsExamplesAndExtensions",
      getAllCactusExamples() +
        getAllCactusExtensions() +
        getAllCactusPackages(),
    );
  }
