...
test(testCase, async (t: Test) => {
  ...
  const approveTokensOutput = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
  ...
  t.equal(approveTokensOutput.data.success, true, "approve() transactionReceipt.status is true OK");

  t.comment("Get account balance");
  const responseBalance = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
  ...
  t.equal(responseBalance.data.callOutput, "100", "balance of account is 100 OK");

  t.comment("Get HashTimeLock contract and account allowance");
  const allowanceOutput = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
    invocationType: EthContractInvocationType.Call,
