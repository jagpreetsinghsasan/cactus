...
test(testCase, async (t: Test) => {
  ...
  const response = await api.apiV1OwnCoordinator(ownHTLCRequest);
  t.equal(response.status, 200, "response status is 200 OK");
  t.equal(response.data.success, true, "response sucess is true");
  t.ok(
    response.data,
    "pluginHTLCCoordinator.apiV1OwnCoordinator() output is truthy OK",
  );
  t.ok(
    response.data.out.transactionReceipt,
    "pluginHTLCCoordinator.apiV1OwnCoordinator() output.transactionReceipt is truthy OK",
  );
  ...
  const response2 = await api.apiV1CounterpartyHTLC(counterpartyHTLCRequest);
  ...
  const response3 = await api.apiV1WithdrawCounterparty(withdrawCounterparty);
  t.equal(response3.status, 200, "response status is 200 OK");
  t.equal(response3.data.success, true, "response sucess is true");
