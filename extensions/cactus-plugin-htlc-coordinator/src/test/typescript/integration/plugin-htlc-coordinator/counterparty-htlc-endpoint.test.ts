...
test(testCase, async (t: Test) => {
  ...
  const htlcCoordinatorApiClient = new HtlcCoordinatorApi(configuration);
  ...
  const response = await htlcCoordinatorApiClient.ownHtlcV1(ownHTLCRequest);
  ...
  const response2 = await htlcCoordinatorApiClient.counterpartyHtlcV1(
    counterpartyHTLCRequest,
  );
