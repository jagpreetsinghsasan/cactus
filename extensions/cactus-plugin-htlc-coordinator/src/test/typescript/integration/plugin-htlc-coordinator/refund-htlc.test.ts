...
test(testCase, async (t: Test) => {
  ...
  const response2 = await htlcCoordinatorApi.counterpartyHtlcV1(
    counterpartyHTLCRequest,
  );
  ...
  try {
    await htlcCoordinatorApi.withdrawCounterpartyV1(withdrawCounterparty);
  } catch (e) {
    const revertReason = e.response.data.error.receipt.revertReason;
