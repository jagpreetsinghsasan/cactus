...
import {
  DefaultApi as HtlcCoordinatorApi,
...
test(testCase, async (t: Test) => {
  ...
  const receiverAccount = await besuTestLedger.createEthTestAccount();

  const web3SigningCredentialReceiverAcc: Web3SigningCredential = {
    ethAccount: receiverAccount.address,
    secret: receiverAccount.privateKey,
    type: Web3SigningCredentialType.PrivateKeyHex,
  } as Web3SigningCredential;
  ...
  const htlcCoordinatorApi = new HtlcCoordinatorApi(configuration);
  ...
  const deployOutTokenSenderAccount = await connector.deployContract({
  ...
  t.ok(deployOutTokenSenderAccount, "deployContract() output is truthy OK");
  t.ok(
    deployOutTokenSenderAccount.transactionReceipt,
    "deployContract() output.transactionReceipt is truthy OK",
  );
  t.ok(
    deployOutTokenSenderAccount.transactionReceipt.contractAddress,
    "deployContract() output.transactionReceipt.contractAddress is truthy OK",
  );
  const tokenAddressSenderAccount = deployOutTokenSenderAccount.transactionReceipt
    .contractAddress as string;

  t.comment("Approve 10 Tokens to HashTimeLockAddress");
  const approveTokensOutputSenderAccount = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
  ...
  t.equal(approveTokensOutputSenderAccount.data.success, true, "approve() transactionReceipt.status is true OK");

  t.comment("Get account balance");
  const responseBalanceSenderAccount = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
  ...
  t.equal(responseBalanceSenderAccount.data.callOutput, "100", "balance of account is 100 OK");

  t.comment("Get HashTimeLock contract and account allowance");
  const allowanceOutput1 = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredential,
    invocationType: EthContractInvocationType.Call,
  ...
  t.equal(allowanceOutput1.data.callOutput, "10", "callOutput() is 10 OK");
  ...
  t.comment("Deploys TestToken via .json file on deployContract function");
  const deployOutTokenReceiverAccount = await connector.deployContract({
    contractName: TestTokenJSON.contractName,
    contractAbi: TestTokenJSON.abi,
    bytecode: TestTokenJSON.bytecode,
    web3SigningCredential: web3SigningCredentialReceiverAcc,
    keychainId,
    constructorArgs: ["50", "token", "2", "TKN"],
    gas: estimatedGas,
  });
  t.ok(deployOutTokenReceiverAccount, "deployContract() output is truthy OK");
  t.ok(
    deployOutTokenReceiverAccount.transactionReceipt,
    "deployContract() output.transactionReceipt is truthy OK",
  );
  t.ok(
    deployOutTokenReceiverAccount.transactionReceipt.contractAddress,
    "deployContract() output.transactionReceipt.contractAddress is truthy OK",
  );
  const tokenAddressReceiverAccount = deployOutTokenReceiverAccount.transactionReceipt
    .contractAddress as string;

  t.comment("Approve 10 Tokens to HashTimeLockAddress");
  const approveTokensOutputReceiverAccount = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredentialReceiverAcc,
    invocationType: EthContractInvocationType.Send,
    methodName: "approve",
    params: [contractAddress, "10"],
    gas: estimatedGas,
  });
  t.equal(approveTokensOutputReceiverAccount.data.success, true, "approve() transactionReceipt.status is true OK");

  t.comment("Get account balance");
  const responseBalanceReceiverAccount = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredentialReceiverAcc,
    invocationType: EthContractInvocationType.Call,
    methodName: "balanceOf",
    params: [receiverAccount.address],
  });
  t.equal(responseBalanceReceiverAccount.data.callOutput, "50", "balance of account is 50 OK");

  t.comment("Get HashTimeLock contract and account allowance");
  const allowanceOutput2 = await besuConnectorApi.apiV1BesuInvokeContract({
    contractName: TestTokenJSON.contractName,
    keychainId,
    signingCredential: web3SigningCredentialReceiverAcc,
    invocationType: EthContractInvocationType.Call,
    methodName: "allowance",
    params: [receiverAccount.address, contractAddress],
  });
  t.equal(allowanceOutput2.data.callOutput, "10", "callOutput() is 10 OKkkkkkkkkkkkkkkkkkkkk");
   ...
   */
   t.comment("Get HashTimeLock contract and account allowance");
   const allowanceOutput3 = await besuConnectorApi.apiV1BesuInvokeContract({
     contractName: TestTokenJSON.contractName,
     keychainId,
     signingCredential: web3SigningCredential,
     invocationType: EthContractInvocationType.Call,
   ...
   t.equal(allowanceOutput3.data.callOutput, "10", "callOutput() is 10 OKiiiiiiiiiiiiiiiiiiiiiii");
  ...
  //   expiration,
  //   hashLock,
  //   tokenAddress: tokenAddressSenderAccount,
  ...
  // const response = await htlcCoordinatorApi.ownHtlcV1(ownHTLCRequest);
  // t.equal(response.status, 200, "response status is 200 OK");
  // t.equal(response.data.success, true, "response sucess is true");
  // t.ok(response.data, "pluginHTLCCoordinator.ownHtlcV1() output is truthy OK");
  // t.ok(
  ...
  // const responseTxId = await besuConnectorApi.apiV1BesuInvokeContract({
  ...
  // const response2 = await htlcCoordinatorApi.counterpartyHtlcV1(counterpartyHTLCRequest);
  // t.equal(response2.status, 200, "response status is 200 OK");
  // t.equal(response2.data.success, true, "response sucess is true");
  ...
  // const response3 = await htlcCoordinatorApi.withdrawCounterpartyV1(withdrawCounterparty);
