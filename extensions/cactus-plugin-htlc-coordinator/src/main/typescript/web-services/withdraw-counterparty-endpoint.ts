...
import { PluginHTLCCoordinator } from "../plugin-htlc-coordinator";
import OAS from "../../json/openapi.json";

export interface IWithdrawCounterpartyOptions {
  logLevel?: LogLevelDesc;
  coordinator: PluginHTLCCoordinator;
}
...
export class WithdrawCounterpartyEndpoint implements IWebServiceEndpoint {
  ...
  constructor(public readonly options: IWithdrawCounterpartyOptions) {
    ...
    Checks.truthy(options.coordinator, `${fnTag} arg options.coordinator`);
  ...
  public getOasPath() {
    return OAS.paths[
      "/api/v1/plugins/@hyperledger/cactus-plugin-htlc-coordinator/withdraw-counterparty"
    ];
  }
  ...
  public async handleRequest(req: Request, res: Response): Promise<void> {
    ...
    try {
      ...
      const resBody = await this.options.coordinator.withdrawCounterparty(
        request,
      );
      res.json(resBody);
    } catch (ex) {
