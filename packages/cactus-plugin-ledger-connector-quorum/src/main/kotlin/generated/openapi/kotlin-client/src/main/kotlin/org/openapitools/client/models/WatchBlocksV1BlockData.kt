/**
 * Hyperledger Cactus Plugin - Connector Quorum
 *
 * Can perform basic tasks on a Quorum ledger
 *
 * The version of the OpenAPI document: 0.0.1
 * 
 *
 * Please note:
 * This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * Do not edit this file manually.
 */

@file:Suppress(
    "ArrayInDataClass",
    "EnumEntryName",
    "RemoveRedundantQualifierName",
    "UnusedImport"
)

package org.openapitools.client.models

import org.openapitools.client.models.OneOfLessThanStringCommaNumberGreaterThan
import org.openapitools.client.models.Web3Transaction

import com.squareup.moshi.Json

/**
 * 
 *
 * @param number 
 * @param hash 
 * @param parentHash 
 * @param nonce 
 * @param sha3Uncles 
 * @param logsBloom 
 * @param stateRoot 
 * @param miner 
 * @param extraData 
 * @param gasLimit 
 * @param gasUsed 
 * @param timestamp 
 * @param propertySize 
 * @param totalDifficulty 
 * @param uncles 
 * @param transactions 
 * @param transactionsRoot 
 * @param receiptsRoot 
 * @param difficulty 
 * @param mixHash 
 */

data class WatchBlocksV1BlockData (

    @Json(name = "number")
    val number: java.math.BigDecimal,

    @Json(name = "hash")
    val hash: kotlin.String,

    @Json(name = "parentHash")
    val parentHash: kotlin.String,

    @Json(name = "nonce")
    val nonce: kotlin.String,

    @Json(name = "sha3Uncles")
    val sha3Uncles: kotlin.String,

    @Json(name = "logsBloom")
    val logsBloom: kotlin.String,

    @Json(name = "stateRoot")
    val stateRoot: kotlin.String,

    @Json(name = "miner")
    val miner: kotlin.String,

    @Json(name = "extraData")
    val extraData: kotlin.String,

    @Json(name = "gasLimit")
    val gasLimit: kotlin.Int,

    @Json(name = "gasUsed")
    val gasUsed: kotlin.Int,

    @Json(name = "timestamp")
    val timestamp: OneOfLessThanStringCommaNumberGreaterThan,

    @Json(name = "size")
    val propertySize: java.math.BigDecimal,

    @Json(name = "totalDifficulty")
    val totalDifficulty: kotlin.String,

    @Json(name = "uncles")
    val uncles: kotlin.collections.List<kotlin.String>,

    @Json(name = "transactions")
    val transactions: kotlin.collections.List<Web3Transaction>,

    @Json(name = "transactionsRoot")
    val transactionsRoot: kotlin.String? = null,

    @Json(name = "receiptsRoot")
    val receiptsRoot: kotlin.String? = null,

    @Json(name = "difficulty")
    val difficulty: kotlin.String? = null,

    @Json(name = "mixHash")
    val mixHash: kotlin.String? = null

)

